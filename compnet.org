...
*** Link-layer Switch
- Forwards and filter frames within LAN 
  - determines output port based on destination MAC address
  - If destination is on the LAN, frame has already been broadcasted on LAN (comes from the LAN), thus switch filters (discards) the frame
...
*** DNS and ARP
- Laptop uses DNS to get the IP address for www.google.com
- DNS query 
  - encapsulated in UDP 
  - encapsulated in IP 
  - encapsulated in.. ooops!
- Laptop needs to get the MAC address of the first-hop router (gateway router)
...
Laptop knows IP address of www.google.com
*** TCP connnection setup
